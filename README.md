...
使用 Spring Boot 3.2 建立 Bitfinex 融資放貸第一版骨架，目前改為每 10 秒輪詢一次 Bitfinex 公開 funding book，以及用 RESTful API 查詢帳戶 wallet 資料；暫時不包含資料庫、私有 API 下單、部位管理與策略執行。
...
- 提供 RESTful API 查詢 Bitfinex 帳戶 wallet 資料`r`n- 提供 RESTful API 查詢 funding offers、credits、loans
...
目前 funding 輪詢不會使用 `BITFINEX_API_KEY` 與 `BITFINEX_API_SECRET`，但帳戶查詢 API 會使用這兩個欄位，且 key 需要有讀取 wallet 權限。
...
```http
GET /api/v1/account/wallets`r`nGET /api/v1/account/funding/offers?symbol=fUSD`r`nGET /api/v1/account/funding/credits?symbol=fUSD`r`nGET /api/v1/account/funding/loans?symbol=fUSD
```

這支 API 會呼叫 Bitfinex 官方 authenticated REST endpoint `POST /v2/auth/r/wallets`，再把結果整理成較易讀的 JSON。`r`nFunding 相關 API 分別對應官方：`r`n`r`n- `POST /v2/auth/r/funding/offers/{Symbol}``r`n- `POST /v2/auth/r/funding/credits/{Symbol}``r`n- `POST /v2/auth/r/funding/loans/{Symbol}``r`n`r`n回傳格式會完整保留 `raw`，並附上 `decoded` 方便在 Swagger 直接閱讀。
