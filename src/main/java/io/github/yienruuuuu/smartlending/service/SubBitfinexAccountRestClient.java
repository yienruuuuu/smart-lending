...
public class SubBitfinexAccountRestClient {
    ...
    public List<BitfinexMovementHistoryEntry> getMovementHistory(String currency, Instant since, Instant until) {
        ...
        if (!root.isArray()) {
            throw new IllegalStateException("Unexpected sub movements response: " + root);
        }
        ...
        log.info("已從 sub account Bitfinex v1 API 取得 movement history。currency={}, count={}", currency, entries.size());
        return entries;
    }
    ...
    public List<BitfinexBalanceHistoryEntry> getBalanceHistory(String currency, Instant since, Instant until) {
        ...
        if (!root.isArray()) {
            throw new IllegalStateException("Unexpected sub balance history response: " + root);
        }
        ...
        log.info("已從 sub account Bitfinex v1 API 取得 balance history。currency={}, count={}", currency, entries.size());
        return entries;
    }
    ...
    public JsonNode postAuthenticatedV1(String apiPath, ObjectNode payload) {
        ...
        try {
            ...
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            headers.set("X-BFX-APIKEY", properties.getSubApiKey());
            headers.set("X-BFX-PAYLOAD", encodedPayload);
            headers.set("X-BFX-SIGNATURE", sign(encodedPayload, properties.getSubApiSecret()));
            ...
            log.error("Sub account Bitfinex v1 REST 請求失敗。endpoint={}", apiPath, exception);
            throw new IllegalStateException("Sub account Bitfinex v1 REST request failed", exception);
        } catch (Exception exception) {
            log.error("解析 sub account Bitfinex v1 回應失敗。endpoint={}", apiPath, exception);
            throw new IllegalStateException("Failed to parse sub account Bitfinex v1 response", exception);
        }
    }
