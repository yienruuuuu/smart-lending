...
public class SubBitfinexFundingAccountRestClient {
    ...
    private List<FundingPositionDto> fetchFundingPositions(String type, String symbol) {
        ...
        JsonNode root = subBitfinexAccountRestClient.postAuthenticated(apiPath, "{}");
        if (!root.isArray()) {
            throw new IllegalStateException("Unexpected sub funding response: " + root);
        }
        ...
        log.debug("已取得 sub funding 資料。type={}, symbol={}, count={}", type, symbol, positions.size());
        return positions;
    }
